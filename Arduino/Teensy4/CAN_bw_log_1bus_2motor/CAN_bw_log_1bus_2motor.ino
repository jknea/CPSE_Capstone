...
void loop() 
{
  for (int i = 0; i < LOG_ITERATIONS; i++) {
    ...
    can1.write(zero_torque_RMD1);
    while(!(received0 && received1)) { wait(1); }
    t1 = micros();
    received0 = 0;
    received1 = 0;
    total_time = t1 - t0;
...
void initCAN() {
  ...
  zero_torque_RMD0.buf[0] = 0xA1;
  zero_torque_RMD1.buf[0] = 0xA1;
  for (int i = 1; i < 8; ++i) {
    zero_torque_RMD0.buf[i] = 0;
    zero_torque_RMD1.buf[i] = 0;
  }
  ...
  can1.setMBFilter(MB1, RMD_CAN_ID1); // Set mailbox 1 to allow CAN ID 0x145 to be collected. 

  can1.onReceive(MB0, canSniff20_MB0); // allows mailbox 0 messages to be received in the supplied callback.
  can1.onReceive(MB1, canSniff20_MB1); // allows mailbox 1 messages to be received in the supplied callback.
