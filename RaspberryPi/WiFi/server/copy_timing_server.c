...
//	int recvbuf = 8;
//	if (setsockopt(master_socket, SOL_SOCKET, SO_RCVBUF, &recvbuf, sizeof(recvbuf)) )
	{
		perror("setsockopt");
		exit(EXIT_FAILURE);
	}
...
//					printf("read %d times\n", (count/8));
					read(sd, imu_buff, fmax(count, 256));
//						ioctl(sd, FIONREAD, &count);
//					}


					for (int i = count - 8, j = 0; i < count; i++, j++) {
